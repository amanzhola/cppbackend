...
RequestHandler::RequestHandler(model::Game& game,
    ...
    , records_loader_{std::move(records_loader)} {
}
...
RequestHandler::StringResponse RequestHandler::HandleGameState(const StringRequest& req) {
    ...
    for (const auto& [object_id, object] : session.GetLostObjects()) {
        json::object object_obj;
        object_obj["type"] = object.type;

        json::array position;
        position.emplace_back(object.position.x);
        position.emplace_back(object.position.y);

        object_obj["pos"] = std::move(position);

        lost_objects_obj[std::to_string(object_id)] = std::move(object_obj);
    }
...
json::value RequestHandler::SerializeMap(const model::Map& map) const {
    return json_serializer::SerializeMap(map, extra_data_);
}
