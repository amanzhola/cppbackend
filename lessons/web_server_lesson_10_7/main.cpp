...
MakeResponse(const http::request<http::string_body>& req) {
    ...
    res.set(http::field::server, "Lesson10_7");
...
void HandleConnection(tcp::socket& socket) {
    ...
    while (true) {
        ...
        std::cout << "Request: " << req.method_string()
                  << " " << req.target()
                  << ", keep_alive=" << std::boolalpha << req.keep_alive() << '\n';

        auto res = MakeResponse(req);
        bool close = !res.keep_alive();

        std::cout << "Response prepared, keep_alive="
                  << std::boolalpha << res.keep_alive()
                  << ", close=" << close << '\n';
        ...
        if (close) {
            std::cout << "Closing connection by protocol rules\n";
            break;
        }
    }
