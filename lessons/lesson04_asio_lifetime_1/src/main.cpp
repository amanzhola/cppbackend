...
public:
    ...
    void Walk(int distance, Callback&& cb) {
        ...
        std::cout << id_ << "> Walk for " << t.count() << "s\n";
        ...
        timer_.async_wait([distance, cb = std::forward<Callback>(cb), this](error_code ec) mutable {
            if (ec) {
                throw std::runtime_error("Wait error: " + ec.message());
            }
            walk_distance_ += distance;
            std::cout << id_ << "> Walked distance: " << walk_distance_ << "m\n";
            cb();
        });
    }
    ...
    void Rotate(int angle, Callback&& cb) {
        ...
        timer_.async_wait([angle, cb = std::forward<Callback>(cb), this](error_code ec) mutable {
            ...
            std::cout << id_ << "> Rotation angle: " << angle_ << "deg\n";
            cb();
        });
    }
...
} // ← здесь r1 и r2 УНИЧТОЖАЮТСЯ

int main() {
    net::io_context io;

    RunRobots(io); // ← роботы уже умерли после выхода
